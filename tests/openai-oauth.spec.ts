...
describe('OpenAI OAuth protocol', () => {
  it('accepts only the matching state on the fixed loopback callback', async () => {
    ...
    expect(accepted.headers.get('content-type')).toBe('text/html; charset=utf-8')
    const successPage = await accepted.text()
    expect(successPage).toContain('<meta name="viewport"')
    expect(successPage).toContain('<link rel="icon" href="data:,">')
    expect(successPage).toContain('data-page="oauth-callback"')
    expect(successPage).toContain('data-state="success"')
    expect(successPage).toContain('data-layout="flat"')
    expect(successPage).toContain('DeepSeek Harness')
    expect(successPage).toContain('@media (prefers-color-scheme: dark)')
    expect(successPage).toContain('<main')
    expect(successPage).toContain('<h1')
    expect(successPage).not.toContain('<script')
    expect(successPage).not.toContain('box-shadow')
    expect(successPage).not.toContain('class="card"')
    await expect(waiting).resolves.toBe('valid-code')
    expect(wrongPage).toContain('data-page="oauth-callback"')
    expect(wrongPage).toContain('data-state="error"')
  })
